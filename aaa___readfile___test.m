function aaa___readfile___test(varargin)
%test the readfile function
%files were written with Notepad++ (which is how encoding was done)
%chars were copied from the command window with (except the UTF8-4 file):
%   clc,fprintf('%c\n',list_of_chars)
%
...
for file=1:4
    %Conver each double array to a cellstr so it can be compared to chars encoded with UTF-16
    %(Matlab) or UTF-8 (Octave).
    x=num2cell(list_of_chars{file}).';
    for m=1:numel(x),x{m}=unicode_to_char(x{m});end
    list_of_chars{file}=x;
end

encoding_list={'ANSI','UTF-8'};
list_of_chars_description={'wide range of chars','32:255 printable chars','nearly all chars',...
    'emoji (mostly two-word UTF-16)'};

part_1='http://web.archive.org/web/';
part_2='id_/https://hjwisselink.nl/FEXsubmissiondata/68780-readfile/aaa___test_file';
timestamp={...
    '20200602120202','20200602120203',NaN,NaN;...
    '20200602120204','20200602120206','20200602120207','20201106120218'};

expected_result_offline=[1 1 0 0;1 1 1 1];
if ~isOctave
    %Matlab
    if ifversion('<',7)
        %Matlab 6.5
        expected_result_online=[...%0=no test, 1=succes, 2=fail, 3=error
            2 1 0 0;...%ANSI;  many chars, <255, all chars, emoji
            2 2 2 2];  %UTF-8; many chars, <255, all chars, emoji
    elseif isunix
        %Confirmed on Ubuntu, probably also true for Mac.
        expected_result_online=[...%0=no test, 1=succes, 2=fail, 3=error
            2 2 0 0;...%ANSI;  many chars, <255, all chars, emoji
            2 1 2 1];  %UTF-8; many chars, <255, all chars, emoji
    elseif ifversion('<',8)%changes somewhere between R2011a and R2015a
        expected_result_online=[...%0=no test, 1=succes, 2=fail, 3=error
            2 2 0 0;...%ANSI;  many chars, <255, all chars, emoji
            2 1 2 1];  %UTF-8; many chars, <255, all chars, emoji
    else
        expected_result_online=[...%0=no test, 1=succes, 2=fail, 3=error
            2 1 0 0;...%ANSI;  many chars, <255, all chars, emoji
            1 1 1 1];  %UTF-8; many chars, <255, all chars, emoji
    end
else
    %Octave
    expected_result_online=[...%0=no test, 1=succes, 2=fail, 3=error
...
for online=0:2
    for file=1:4
        for encoding=1:2
            ...
            if file==4 && encoding==1,continue,end
            try
                if online==1
                ...
                elseif online==2
                    ...
                    %Trigger an error once with the debug hook to cause the no download path.
                    HJW___test_suit___debug_hook_data=struct('action','error','data',{' '});
                else
                    fn=sprintf('aaa___test_file_%s_%d.txt',...
                        encoding_list{encoding},file);
                    OnlineTag='';
                    if RunTestHeadless && ~exist(fn,'file')
                        %Called as a sub-test; skip test and write expected result.
                        results(encoding,file,online+1)=expected_result(encoding,file,online+1);
                        continue
                    end
                end
                ...
                if isequal(txt,list_of_chars{file})
                    ...
                    L=false(size(txt));
                    for c=1:numel(txt)
                        L(c)=isequal(txt{c},list_of_chars{file}{c});
                    end
                    percentage_correct=floor(100*mean(L));%floor() prevents mis-allignment for 100%
                    fprintf('Read  %02d%% correct: %s (%s)%s\n',...
                        percentage_correct,encoding_list{encoding},list_of_chars_description{file},OnlineTag)
...
function fail_pass=test_syntax
fail_pass='pass';
try
...
try
    options=struct('print_to_con',true);
    non_existent=tempname;while exist(non_existent,'file'),non_existent=tempname;end
    readfile(non_existent,options)
catch
    ME=lasterror; %#ok<LERR>
    if ~strcmp(ME.identifier,'HJW:readfile:ReadFail')
        fail_pass='fail';
        return
    end
end
try
    readfile(non_existent,'print_to_con',true)
catch
    ME=lasterror; %#ok<LERR>
    if ~strcmp(ME.identifier,'HJW:readfile:ReadFail')
        fail_pass='fail';
