function aaa___test___readfile(varargin)
%test the readfile function
%files were written with Notepad++ (which is how encoding was done)
%chars were copied from the command window with:
%   clc,fprintf('%c\n',list_of_chars)
%
...
[fail_pass,BackupPref]=test_syntax;
...
if BackupPref
    old_setting=getpref('HJW',...%toolbox name or author initials as group ID
        ['UTF8_to_str___',...%function group
        'behavior__char_geq256'],...%preference name
        -1);
    setpref('HJW','UTF8_to_str___behavior__char_geq256',0)
end
...
list_of_chars{2}=[32:126 160:255 32 32 32]';
list_of_chars{3}=[...
...
encoding_list={'ANSI','UTF-8'};
list_of_chars_description={'wide range of chars','32:255 printable chars','nearly all chars'};

part_1='http://web.archive.org/web/';
part_2='id_/https://hjwisselink.nl/FEXsubmissiondata/68780-readfile/aaa___test_file';
timestamp={...
    '20200602120202','20200602120203',NaN;...
    '20200602120204','20200602120206','20200602120207'};

if ~isOctave
    %Matlab
    if ifversion('<',7)
        %Matlab 6.5
        expected_result_offline=[...%0=no test, 1=succes, 2=fail, 3=error
            2 1 0;...%ANSI;  many chars, <255, all chars
            1 1 1];  %UTF-8; many chars, <255, all chars
        expected_result_online=expected_result_offline;
        %results below appear not to be true when reading from the WBM
        % %some trailing bits (e.g. 129 and 141) are converted to '?'
        % expected_result_online=[...%0=no test, 1=succes, 2=fail, 3=error
        %     2 1 0;...%ANSI;  many chars, <255, all chars
        %     2 2 2];  %UTF-8; many chars, <255, all chars
    elseif ifversion('<=','R2011a')
        expected_result_offline=[...%0=no test, 1=succes, 2=fail, 3=error
            1 1 0;...%ANSI;  many chars, <255, all chars
            1 1 1];  %UTF-8; many chars, <255, all chars
        expected_result_online=[...%0=no test, 1=succes, 2=fail, 3=error
            2 2 0;...%ANSI;  many chars, <255, all chars
            1 1 1];  %UTF-8; many chars, <255, all chars
    else
        expected_result_offline=[...%0=no test, 1=succes, 2=fail, 3=error
            1 1 0;...%ANSI;  many chars, <255, all chars
            1 1 1];  %UTF-8; many chars, <255, all chars
        %online has same support as offline
        expected_result_online=expected_result_offline;
    end
else
    %Octave
    expected_result_offline=[...%0=no test, 1=succes, 2=fail, 3=error
        2 1 0;...%ANSI;  many chars, <255, all chars
        2 1 2];  %UTF-8; many chars, <255, all chars
        %online has same support as offline
        expected_result_online=expected_result_offline;
end
expected_result=cat(3,expected_result_offline,expected_result_offline,expected_result_online);

global HJW___test_suit___trigger_error_in_tempname
HJW___test_suit___trigger_error_in_tempname=0;

results=zeros(size(expected_result));%pre-allocate
clc
for online=0:2
    for file=1:3
        for encoding=1:2
            if file==3 && encoding==1,continue,end
            try
                if online==1
                    fn=sprintf('%s%s%s_%s_%d.txt',part_1,timestamp{encoding,file},part_2,...
                        encoding_list{encoding},file);
                    OnlineTag=' [online file]';
                elseif online==2
                    fn=sprintf('%s%s%s_%s_%d.txt',part_1,timestamp{encoding,file},part_2,...
                        encoding_list{encoding},file);
                    OnlineTag=' [online file, no temp download]';
                    HJW___test_suit___trigger_error_in_tempname = 1;
                else
                    fn=sprintf('aaa___test_file_%s_%d.txt',...
                        encoding_list{encoding},file);
                    OnlineTag='';
                end
                
                txt=double(cell2mat(readfile(fn)));
                if isequal(txt,list_of_chars{file})
                    fprintf('Read   correct: %s (%s)%s\n',...
                        encoding_list{encoding},list_of_chars_description{file},OnlineTag)
                    results(encoding,file,online+1)=1;
                else
                    fprintf('Read incorrect: %s (%s)%s\n',...
                        encoding_list{encoding},list_of_chars_description{file},OnlineTag)
                    results(encoding,file,online+1)=2;
                end
            catch
                fprintf('[error] Read failed: %s (%s)%s\n',...
                    encoding_list{encoding},list_of_chars_description{file},OnlineTag)
                if online==1 && ~isnetavl
                    fprintf('        note: no internet connection available\n')
                end
                results(encoding,file,online+1)=3;
            end
        end
    end
end

if isequal(results,expected_result)
    ThrowError=false;
    fprintf('\nTest result matches expectation.\n\n');
else
    ThrowError=true;
    fprintf('\nTest result did *not* match expectated result.\n\n');
end

if BackupPref
    %restore
    if old_setting==-1
        rmpref('HJW','UTF8_to_str___behavior__char_geq256')
    else
        setpref('HJW','UTF8_to_str___behavior__char_geq256',old_setting)
    end
end
if ThrowError
...
function [fail_pass,BackupPref]=test_syntax
fail_pass='pass';
try
    readfile(struct('name','foo.txt'));
    fail_pass='fail';
    return
catch
end
try
    thisfile=readfile([mfilename '.m']);
    if size(thisfile,1)==1
        error('result should be nx1, not 1xn')
    end
catch
    fail_pass='fail';
    return
...
%retrieve the default behavior for UTF8_to_str
try
    [unicode,flag]=UTF8_to_str('UTF8',1); %#ok<ASGLU>
    BackupPref= flag.default==2;
catch
    %This happens if the flag output is not set, or if readfile doesn't use this function anymore.
    BackupPref=false;
end
end
...
%shadow the tempname function so it can throw an error

%use a global to either trigger an error during testing, or return a valid temp name
global HJW___test_suit___trigger_error_in_tempname
if isempty(HJW___test_suit___trigger_error_in_tempname)
    HJW___test_suit___trigger_error_in_tempname=0;
end
if HJW___test_suit___trigger_error_in_tempname>1
    HJW___test_suit___trigger_error_in_tempname=HJW___test_suit___trigger_error_in_tempname-1;
    error('trigger error here')
else
    dict=char(['a':'z' '0':'9' 'A':'Z']);
    N=15;%randstring length
    randstring=dict( floor( (numel(dict)-1)*rand(1,N) ) +1);
    
    str=fullfile(tempdir,randstring);
end
end
